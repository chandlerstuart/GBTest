/**
  ...
  * @date -           MAY 11, 2016
  * @description -    Trigger for POInvoice__c
  * #############Change Log#############
  * Modified by     Version     Change Date
  **/

trigger POInvoiceTrigger on POInvoice__c (before insert, after insert, before update, after update, before delete, after delete, after undelete) {
	OrgSettings__c orgSettings = OrgSettings__c.getInstance();
    if( orgSettings<> null && !orgSettings.TriggersActive__c ) return;

    AP49_POInvoiceHandler handler = new AP49_POInvoiceHandler();
