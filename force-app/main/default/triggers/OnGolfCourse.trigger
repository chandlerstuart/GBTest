...
*/
trigger OnGolfCourse on GolfCourse__c (after insert) {
    ...
    */
    if (Trigger.isInsert && Trigger.isAfter) {
        Map<Id, SObject> processMap = S2SExternalSharingHelper.filterRecords(Trigger.new, 'GolfCourse__c', 'Account__c', 'Account');
        List<GolfCourse__c> processList = new List<GolfCourse__c>();
        Set<Id> recordIds = new Set<Id>();

        for (GolfCourse__c h : Trigger.new){
        ...
        if (null != processList && !processList.isEmpty()){
            ...
            h.shareRecordsFromTrigger(processList, 'Account__c', 'GolfCourse__c', true);    
        
        }
        
        if (null != recordIds && !recordIds.isEmpty()){

            S2SExternalSharingHelper.scheduleSharing(recordIds, 'GolfCourse__c','Account__c',NULL);
        }
