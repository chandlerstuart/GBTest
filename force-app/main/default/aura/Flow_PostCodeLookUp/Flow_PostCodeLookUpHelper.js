({
    startSearch : function(component, event, helper) {
        ...
        var url = 'http://services.postcodeanywhere.co.uk/Capture/Interactive/Find/v1.10/json3.ws';
        var params = '';
        params += "&Key=" + apiKey;
    ...
    retrieveAddress : function(component,event,helper){
        var apiKey = $A.get('$Label.c.LoqateAPIKey');
        if(!apiKey) return;
        var url = 'http://services.postcodeanywhere.co.uk/Capture/Interactive/Retrieve/v1.00/json3.ws';
        var params = '';
        params += "&Key=" + apiKey;
            ...
            if(http.readyState == 4 && http.status == 200) {
                ...
                if (response.Items.length == 1 && typeof(response.Items[0].Error) != "undefined") {
                    ...
                    if (response.Items.length == 0)
                        ...
                        //Map response to address values
                        component.set("v.street",streetString);
                        component.set("v.city",response.Items[0].City);
