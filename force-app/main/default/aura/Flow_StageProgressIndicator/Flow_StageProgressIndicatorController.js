({
   init : function(component, event, helper) {
       if(component.get("v.ShowBorder")){
           var article = component.find('articleId');
           $A.util.addClass(article,'slds-card');
       }
       var headerString = component.get("v.Header");
       if(headerString==null || headerString == ''){
           if(component.get("v.IsModal")){
               var modalHeader = component.find('modalHeaderId');
               $A.util.addClass(modalHeader,'slds-modal__header_empty');
           }
       }
